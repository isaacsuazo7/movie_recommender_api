...
# Se calculan los valores del coseno a partir de los datos del coseno      
def transform_data(data_combine, data_plot):
        count = CountVectorizer(stop_words='english')
        count_matrix = count.fit_transform(data_combine['combine'])

        tfidf = TfidfVectorizer(stop_words='english')
        tfidf_matrix = tfidf.fit_transform(data_plot['original_title'])
...
# Se recomeinda la pelicula con todos los datos anteriormente obtenidos
def recommend_movies(title, data, combine, transform):
        ...
        movie_overview = data['overview'].iloc[movie_indices]
        movie_cast = data['cast'].iloc[movie_indices]

        recommendation_data = pd.DataFrame(columns=['id','name', 'genres', 'overview', 'cast'])

        recommendation_data['id'] = movie_id
        recommendation_data['name'] = movie_title
        recommendation_data['genres'] = movie_genres
        recommendation_data['overview'] = movie_overview
